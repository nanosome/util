...
package nanosome.util {
	...
	/**
	 * <code>EnterFrame</code> provides a high-performant way to trigger the common
	 * enter-frame event dispatched by DisplayObject's.
	 * 
	...
	public class EnterFrame {
		...
		public static function add( method: Function, executeImmediately: Boolean = false,
			...
			if( !_listening ) {
				_listening = true;
				_helper.addEventListener( Event.ENTER_FRAME, _list.executeStraight );
			}
		...
		private static function removeListeners(): void {
			_listening = false;
			_helper.removeEventListener( Event.ENTER_FRAME, _list.execute );
		}
	}
}
